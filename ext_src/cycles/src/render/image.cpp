                                      ...
                                      ImageMetaData& metadata)
{
	...
	if(spec.format == IMG::TypeDesc::HALF)
...
template<IMG::TypeDesc::BASETYPE FileFormat,
...
{
	const StorageType alpha_one = (FileFormat == IMG::TypeDesc::UINT8)? 255 : 1;
	ImageInput *in = NULL;
	...
	if(in) {
		...
		if(FileFormat == IMG::TypeDesc::FLOAT) {
		...
		else if(FileFormat == IMG::TypeDesc::UINT8) {
                                     ...
                                     Progress *progress)
{
	...
	if(type == IMAGE_DATA_TYPE_FLOAT4) {
		...
		if(!file_load_image<IMG::TypeDesc::FLOAT, float>(img,
		                                                 type,
		                                                 texture_limit,
		                                                 *tex_img))
		{
		...
		if(!file_load_image<IMG::TypeDesc::UINT8, uchar>(img,
		                                                 type,
		                                                 texture_limit,
		                                                 *tex_img))
		{
		...
		if(!file_load_image<IMG::TypeDesc::HALF, half>(img,
		                                               type,
		                                               texture_limit,
		                                               *tex_img)) {
			/* on failure to load, we set a 1x1 pixels pink image */
